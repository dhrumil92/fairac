...
const SessionPage = () => {
  ...
  // Forms
  const [sessionType, setSessionType] = useState('unlimited');
  ...
  const handleStartSession = async (e) => {
    ...
    try {
      ...
      if (sessionType !== 'unlimited' && targetValue) payload.target_value = parseFloat(targetValue);
    ...
    <div className="font-body text-sm bg-[#0F1729] text-[#F8FAFC] min-h-screen flex">
      ...
      <main className="flex-1 min-h-screen relative md:ml-64">
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
          {error && <div className="p-4 bg-[#FF6B6B]/10 border border-[#FF6B6B]/20 text-[#FF6B6B] rounded-xl font-medium">⚠️ {error}</div>}
          {success && <div className="p-4 bg-[#00D4AA]/10 border border-[#00D4AA]/20 text-[#00D4AA] rounded-xl font-medium">✅ {success}</div>}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-4 space-y-8">
                ...
                <section className={`${glassCardClasses} rounded-2xl p-6 relative overflow-hidden group transition-all hover:border-[#6C63FF]/50 ${activeSession ? 'opacity-70 pointer-events-none' : ''}`}>
                    ...
                    <form className="space-y-6" onSubmit={handleStartSession}>
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <select 
                            className="w-full bg-[#0F1729] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-[#6C63FF] focus:border-transparent outline-none appearance-none cursor-pointer"
                            value={sessionType}
                      ...
                      {sessionType !== 'unlimited' && (
                        <div className="space-y-2">
                          <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">
                            {sessionType === 'duration' ? 'Target Hours' : 'Target Budget'}
                          </label>
                          <input 
                            type="number"
                            step="0.1"
                            ...
                            placeholder={sessionType === 'duration' ? 'e.g. 2.5' : 'e.g. 50'}
                            disabled={!!activeSession}
                          />
                        </div>
                      )}
                      
                      <button type="submit" disabled={!!activeSession} className={`w-full py-4 bg-[#6C63FF] text-white font-bold rounded-xl shadow-lg ${glowPrimary} flex items-center justify-center gap-2 hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed`}>
                ...
                {activeSession && myParticipant?.status === 'invited' && (
                  <section className={`${glassCardClasses} rounded-2xl border-l-4 border-l-[#6C63FF]/30 p-6 relative`}>
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-full border-2 border-[#6C63FF]/40 p-0.5 flex items-center justify-center bg-slate-800 text-white font-bold">
                        i
                      </div>
                      <div className="flex-1">
                        <h4 className="font-headline font-bold text-white text-sm mb-1">Session Invitation</h4>
                        <p className="text-xs text-slate-400 leading-relaxed">
                          <span className="text-white font-medium">A roommate</span> invited you to join a running AC session in <span className="text-[#6C63FF] font-semibold">{myRoom.room_name || myRoom.room_no}</span>.
                        </p>
                        <div className="flex gap-2 mt-4">
                          <button onClick={() => handleParticipantAction('accept')} className="flex-1 py-2 bg-[#00D4AA]/10 hover:bg-[#00D4AA]/20 text-[#00D4AA] text-[11px] font-bold rounded-lg border border-[#00D4AA]/20 transition-colors uppercase tracking-wider">
                            Accept
                          </button>
                          <button onClick={() => handleParticipantAction('reject')} className="flex-1 py-2 bg-transparent hover:bg-[#FF6B6B]/10 text-[#FF6B6B] text-[11px] font-bold rounded-lg border border-[#FF6B6B]/20 transition-colors uppercase tracking-wider">
                            Reject
                          </button>
                        </div>
                      </div>
                  ...
                  <section className={`${glassCardClasses} rounded-3xl overflow-hidden shadow-2xl relative min-h-[500px] flex flex-col`}>
                    ...
                    <div className="p-8 border-b border-white/5 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-slate-400 text-sm">{myRoom.room_name || `Room ${myRoom.room_no}`} • Auto-Meter Linked</p>
                      </div>
                          ...
                          {activeSession.participants?.map(p => (
                            <div key={p.u_id} className={`flex items-center justify-between ${p.status === 'invited' ? 'opacity-60' : ''}`}>
                              ...
                              <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${
                                p.status === 'accepted' ? 'bg-[#00D4AA]/10 text-[#00D4AA]' : 
