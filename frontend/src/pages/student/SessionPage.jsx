                      ...
                      <div className="relative z-10">
                        ...
                        <form className="space-y-6" onSubmit={handleStartSession} noValidate>
                          ...
                          {sessionType !== 'unlimited' && (
                            ...
                            <button 
                              type="submit" 
                              disabled={user?.is_active === false || myRoom?.room_active === false || myRoom?.hostel_active === false || deviceStatus?.status !== 'online'}
                              className={`w-full py-3.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                                (user?.is_active === false || myRoom?.room_active === false || myRoom?.hostel_active === false || deviceStatus?.status !== 'online') 
                                ? 'bg-slate-700 text-slate-400 cursor-not-allowed' 
                                : 'bg-[#00D4AA] text-[#0F1729] hover:bg-[#00E6B8] hover:shadow-[0_0_20px_rgba(0,212,170,0.4)]'
                              }`}
                            >
                            <span className="material-symbols-outlined">bolt</span>
                            {deviceStatus?.status !== 'online' ? (deviceStatus ? 'AC is Offline' : 'Connecting to AC...') : 'Start Session Now'}
                          </button>
