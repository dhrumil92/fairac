...
RTC_DATA_ATTR float simulatedKwh = 0.0; // Survives software reboots!
...
void loop() {
  ...
  // ---------------------------------------------------------
  // WiFi LED Status Indicator & Auto-Reconnect Logic
  // ---------------------------------------------------------
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (disconnectedStartMillis == 0) {
        disconnectedStartMillis = currentMillis; // Start the timer
    }
    
    if (currentMillis - disconnectedStartMillis > 30000) {
        Serial.println("\n[ERROR] WiFi lost for 30 seconds! Attempting to reconnect...");
        WiFi.disconnect();
        WiFi.reconnect();
        disconnectedStartMillis = currentMillis; // Reset the timer to try again in another 30 seconds
    }
  }
  ...
  if (isAcOn && (currentMillis - previousTelemetryMillis >= telemetryInterval)) {
    ...
    // Simulate variable wattage (e.g., between 1350W and 1650W)
    simulatedPowerW = random(1350, 1651);

    // Simulate consuming power
